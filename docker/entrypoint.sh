...
"once")
    echo "🔄 单次执行"
    exec /app/.venv/bin/python -m trendradar
    ;;
"cron")
    ...
    # 生成 crontab
    echo "$CRON_EXPR cd /app && /app/.venv/bin/python -m trendradar" > /tmp/crontab
    ...
    if [ "${ENABLE_WEBSERVER:-false}" = "true" ]; then
        echo "🌐 启动 Web 服务器..."
        /app/.venv/bin/python manage.py start_webserver
        ...
        if [ "$WEBSERVER_WATCHDOG_ENABLED" = "true" ] || [ "$WEBSERVER_WATCHDOG_ENABLED" = "1" ] || [ "$WEBSERVER_WATCHDOG_ENABLED" = "yes" ] || [ "$WEBSERVER_WATCHDOG_ENABLED" = "on" ]; then
            ...
            (
                while true; do
                    ...
                    /app/.venv/bin/python manage.py webserver_autofix
                done
            ) &
